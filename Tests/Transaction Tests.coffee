...
tape "transactions are monotonic", (test) ->
  facts = Facts()
  facts.transact [ ["advance", 1, i, yes] ] for i in [100..0]
...
tape "transaction report has expected keys", (test) ->
  facts = Facts()
  report = facts.transact [ ["advance", 1, "name", "Ursula Franklin"] ]
...
tape "subsequent transaction generate new entries in the database history", (test) ->
  ...
  test.same facts.history.count(), 2
  facts.transact [["advance", 2, "en", "Goodbye moon"]]
...
tape "transact one reversal", (test) ->
  facts = Facts()
  report = facts.transact [ ["reverse", 1, "name", "Ursula Franklin"] ]
  test.same facts.history.count(), 2
  test.end()

tape "transact one unknown", (test) ->
  facts = Facts()
  report = facts.transact [ ["unknown", 1, "name", "Ursula Franklin"] ]
  test.same facts.history.count(), 2
  test.end()
...
tape "last transaction is at the beginning of the stack of datoms", (test) ->
  facts = Facts()
  facts.transact [ ["advance", 1, "name", "Ursula Franklan"] ], 123
  facts.transact [ ["advance", 1, "name", "Ursula Franklen"] ], 456
  facts.transact [ ["advance", 1, "name", "Ursula Franklin"] ], 789
...
tape "transact raw access", (test) ->
  facts = Facts()
  report = facts.transact [ ["advance", 1, "name", "Ursula Franklin"] ], 123
  test.same facts.history.count(), 2
...
tape "database value after transaction", (test) ->
  facts = Facts()
  facts.transact [ ["advance", 1, "name", "Ursula Franklan"] ], 123
  facts.transact [ ["advance", 1, "name", "Ursula Franklin"] ], 456
