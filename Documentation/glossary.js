...
(function() {
  ...
  delegate2("input", "form", function(event) {
    ...
    return window.requestAnimationFrame(function() {
      var input, missingInputs, requiredInputs, _i, _len;
      console.info(requiredInputs = Array.apply(void 0, form.querySelectorAll("[required]:not(:disabled)")));
      for (_i = 0, _len = requiredInputs.length; _i < _len; _i++) {
        input = requiredInputs[_i];
        console.info("in", input.name, input.value.trim() === "");
      }
      console.info("missing", missingInputs = (function() {
        var _j, _len1, _results;
        _results = [];
        for (_j = 0, _len1 = requiredInputs.length; _j < _len1; _j++) {
          input = requiredInputs[_j];
          if (input.value.trim() === "") {
            _results.push(input);
          }
        }
  ...
  render = function() {
    ...
    entities = facts.query({
      "in": facts.database(),
      where: function(id) {
        return __indexOf.call(glossary.words, id) >= 0;
      }
    });
  ...
  autolink = function(string, glossary) {
    ...
    glossaryWords = (function() {
      ...
      for (_i = 0, _len = words.length; _i < _len; _i++) {
        word = words[_i];
        if (__indexOf.call(glossary.words, word) >= 0) {
          _results.push(word);
        }
      }
      return _results;
